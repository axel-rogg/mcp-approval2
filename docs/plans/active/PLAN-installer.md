...
- **CF Free-Plan limitiert Zone-Rulesets pro Phase auf 1**: ein zweites `cloudflare_ruleset` mit `kind="zone"` + `phase="http_ratelimit"` schlägt mit `code:20217` fehl wenn die Zone bereits ein Ratelimit-Ruleset hat. Workaround: In-Process-Rate-Limiter im App-Code (siehe `mcp-knowledge2/src/middleware/rate_limit.ts`) oder CF-Ruleset-Sharing mit existierendem Worker. TF-Variable `enable_cf_zone_ratelimit=false` als Default gate.
- **pg-boss v10+ verlangt explizites `createQueue()` vor `work()`/`schedule()`** (knowledge2-Deploy-Bug 2026-05-17). Ohne `createQueue('<name>')` Boot-Time-Call fatal `Queue uploads.sweep_expired not found` beim Worker-Start. Fix in [`apps/server/src/crons/runner.ts`](https://github.com/axel-rogg/mcp-knowledge2/blob/main/apps/server/src/crons/runner.ts): vor `boss.schedule()` für jeden Queue-Namen ein `await boss.createQueue(name)` (idempotent in pg-boss).
- **Postgres 15+ `public` schema ownership** (approval2 + knowledge2 Bootstrap 2026-05-17). In PG-15+ ist `public` owned by `pg_database_owner` (= DB-Owner-Role), nicht mehr direkt schreibbar für andere Rollen. Migrations als `<service>_admin` scheitern mit `permission denied for schema public` bis der DB-Owner (Neon-Side: die `<service>_app`-Role ist Owner) explizit grantet. Bootstrap-SQL **als DB-Owner-Role** ausführen:
  ```sql
  GRANT ALL ON SCHEMA public TO <service>_admin;
  ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO <service>_admin;
  ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO <service>_admin;
  ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON FUNCTIONS TO <service>_admin;
  ```
  Phase 5 des Installers (siehe §3.5) muss den SQL-Block + die Hinweis-Note zur Owner-Role enthalten.
- **R2 EU-Jurisdiction-Endpoint** (approval2-Deploy-Bug 2026-05-17). Buckets die mit `jurisdiction="eu"` provisioniert sind verlangen `BLOB_ENDPOINT=https://<account>.eu.r2.cloudflarestorage.com` (mit `.eu.`-Segment). Ohne `.eu.` antwortet R2 mit `403 Bucket not found`. Doc-fixed in [`terraform/environments/privat/r2-blob.tf`](../../../terraform/environments/privat/r2-blob.tf) outputs. Phase 3 muss den Endpoint pro Bucket-Jurisdiction unterscheiden.
- **Docker workspace-local node_modules** (approval2-Deploy-Bug 2026-05-17). `Dockerfile.server` Build-Stage muss nach `COPY` der Sources ein `npm install --no-save` ausführen damit Workspace-lokale `node_modules` (z.B. `apps/web/node_modules/marked`) korrekt rekonstruiert werden. Ohne den Re-Install scheitert `tsc --noEmit && vite build` mit `Cannot find module 'marked'`. Hängt mit npm-workspaces + Docker-Layer-Cache-Interaktion zusammen.
- **`hashicorp/google` Provider 6.x Multi-Region-KMS-Bug** (KMS-Provisioning 2026-05-17). `google_kms_key_ring` mit `location="eu"` (multi-region) schlägt mit `KMS_RESOURCE_NOT_FOUND_IN_LOCATION, request misrouted to global` fehl. Workaround: Single-Region `europe-west3` (siehe [docs/privat.md §9.3](../../privat.md)). Cost-identisch, Failover für Solo-Pilot überdimensioniert. Switch zurück möglich sobald Provider-Bug gefixt ist.
- **`fly.toml` braucht `release_command` für Auto-Migrations** (approval2-Deploy-Bug 2026-05-17). Ohne `[deploy] release_command = "sh -c 'DATABASE_URL=$DATABASE_ADMIN_URL npm run db:migrate'"` werden Schema-Migrations nie ausgeführt — App startet, antwortet auf `/health` mit 200, aber jeder DB-schreibende Endpoint (z.B. `/oauth/register`) wirft 500 weil Tabellen fehlen. Migration-Tracking via `_migrations`-Tabelle macht den Call idempotent.
- **Health-Check-Triage: load-bearing vs opportunistic** (knowledge2-Deploy-Bug 2026-05-17). `/health/ready` muss differenzieren zwischen DB (load-bearing → 503 bei Fail) und Blob (opportunistic → 200 mit `status="degraded"` bei Fail). Andernfalls killt eine R2-Outage den Fly-Proxy obwohl die App lese-/schreib-fähig für Postgres-only-Operationen wäre. Fix in [`apps/server/src/routes/health.ts`](https://github.com/axel-rogg/mcp-knowledge2/blob/main/apps/server/src/routes/health.ts).
