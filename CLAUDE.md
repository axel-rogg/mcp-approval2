...
> **⚙️ Update 2026-05-18 spätabend — Tool-Defaults v2 COMPLETE.** 6 Phasen live (Commits `6a70010` A → `0362c7f` F). Resolver mergt Per-User-Defaults aus `user_server_tool_defaults` vor jedem `tools/call` mit Args-WIN; WYSIWYS-Attribution pro Feld (`from profile=prod` / `from user-input`) in pending_approvals.defaults_applied (Mig 0027). **Typed Values** in `value_json jsonb` + Schema-Validation gegen Tool-inputSchema. **Multi-Profile** pro `(user × sub_mcp_name)` mit `__profile`-Arg als Per-Call-Override + Profile-Switcher-Bar in PWA. **`tools.help`**-Tool (read) liefert Schema + effective Defaults + Hints + Profile pro Tool; **`_meta.defaults_summary`** aggregiert pro Tool in `tools/list`. **Hints + Elicitation-Hook** (Default OFF, Capability-gated). **Per-User-Isolation** strikt via RLS + `user_id` in jedem PK. **prefs.*-Tools deprecated** mit Banner, Werte schreiben in tote `user_tool_prefs` (Mig 0030 dropped sie ab 2026-06-17). 707 server + 53 web Tests grün. AI-Agent-Guide: [docs/guides/agent-guide-tool-defaults.md](docs/guides/agent-guide-tool-defaults.md). Plan: [docs/plans/active/PLAN-tool-defaults-v2.md](docs/plans/active/PLAN-tool-defaults-v2.md).
>
...
>
> **🤝 Update 2026-05-18 — Group-Sharing Phase 2 Code-Complete, Deploy-pending.** 8 Items + 4 Followups gestaffelt (P2-1 bis P2-8 + F2.1–F2.4). 13 Group-Sharing-Tools (vs 5 Phase-1) inkl. `groups.transfer_ownership` (danger), `groups.invite_email`, `shares.list_my_shares`, `docs.share_with_group`. KC2-Side: Migrationen 0023 (Cross-Member-Visibility), 0024 (group-write scope), 0025 (owner-transfer policy-split), 0026 (rewrap_jobs async-queue). Bidirectional Invite (`invites.target_group_id`) — signup+group-add Ceremony. PWA: Storage-Detail Share-Modal mit Cascade-Preview + #/admin/groups Shared-with-me Section + Transfer-Owner-UI. GH-Actions Cron `*/2` fuer `rewrap-tick`. GcsWormSink + TF-Bucket fuer SOC-2/ISO-27001 (dormant; `gcs_audit_enabled=true` zum Aktivieren). **Deploy-Blocker:** Neon-Role-Grant noetig vor `fly deploy -a mcp-knowledge2` — Operator-Step in [knowledge2/docs/runbooks/runbook-neon-role-grants.md](https://github.com/axel-rogg/mcp-knowledge2/blob/main/docs/runbooks/runbook-neon-role-grants.md). Full Status: [docs/plans/active/PLAN-sharing-phase-2.md](docs/plans/active/PLAN-sharing-phase-2.md). Operator-Sequenz: [docs/runbooks/operator-checklist-2026-05-18.md](docs/runbooks/operator-checklist-2026-05-18.md).
>
> **🤝 Update 2026-05-17 spätabend — Group-Sharing Phase 1 LIVE.** Cross-Repo-Feature: KC2 mit Migration 0019+0020 (groups, group_members, share_grants extension, RLS-Patch) + AAD-v2 + KMS wrapBytes + Lazy-Migration + readObject/updateObject Dispatch + Group-CRUD + Cascade-Hook. approval2 mit HttpKnowledgeAdapter (8 neue Methoden), KnowledgeService-Erweiterung (8 audited Methoden), Tool-Surface (5 Kern-Tools: `groups.create/list/add_member/remove_member` + `skills.share_with_group`), PWA Sub-Tab `#/admin/groups` mit Group-CRUD + Member-Mgmt. Crypto: Per-Object-DEK + Group-Master (KMS-wrapped, 5min cache) + Member-Wrap mit AAD-Domain-Separation. Member-Remove rotiert Master in einer TX (Hard-Cap 1000 Grants). Cascade-Hook bei `addRef(role='resource')` triggert wenn `parent.subtype='skill_manifest'` + `parent.cascade_on_share=TRUE`. Test-Coverage: 19 group-crypto Unit + 5 AAD-Domain-Separation + 15 RLS-Integration (Testcontainer). Family-Modus: dormant (kein Group, kein User-Impact). Self-Host-für-Freunde + Corporate-VPC: ready für Use-Case.
>
...
| **[PLAN-sharing-phase-2.md](docs/plans/active/PLAN-sharing-phase-2.md)** | ✅ **Code-Complete 2026-05-18** (Deploy-pending) | 8 P2-Items + 4 Followups (Owner-Transfer, Co-Edit, Cross-Member-Visibility, Bidirectional Invite, Async-Rewrap-Worker, WORM-Audit, Cascade-Preview-UI, Email-Invite-MVP). |
| **[PLAN-tool-defaults-v2.md](docs/plans/active/PLAN-tool-defaults-v2.md)** | ✅ **COMPLETE 2026-05-18** (Phases A-F) | Per-User-Tool-Defaults mit Multi-Profile (`__profile`-Override), typed Storage (Mig 0027/0028/0029), `tools.help` + `_meta.defaults_summary`, Hints + Elicitation-Hook, RLS-Isolation. Agent-Guide: [agent-guide-tool-defaults.md](docs/guides/agent-guide-tool-defaults.md). |
